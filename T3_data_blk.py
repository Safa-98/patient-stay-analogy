...
class Task1Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filename = "T3_ID+DSEQ_BLK.txt"): #T2_ID+SEQ_ICD_1800.txt #T3_ID+DSEQ_ICD_1800.txt
        super(Task1Dataset).__init__()
        #self.mode = mode
        self.raw_data = load_data(filename = filename)
        #self.prepare_data()
        self.set_analogy_classes()
        self.demo_dict = json.load(open(os.path.join('processed_blk_T3/files/demo_dict.json'), 'r')) #processed_icd_SEQ
        self.vector_dict = json.load(open('processed_blk_T3/files/vector_dict.json', 'r'))

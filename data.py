...
class Task1Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filename = "T1_IDENTITY_200.txt"): 
        ...
        self.demo_dict = json.load(open(os.path.join('processed_id/files/demo_dict.json'), 'r'))
        self.vector_dict = json.load(open('processed_id/files/vector_dict.json', 'r'))
    ...
    def set_analogy_classes(self):
        self.analogies = []
        self.all_words = set()
        for i, (stay_a_i, stay_b_i, patient_a_i) in enumerate(self.raw_data):
            self.all_words.add(stay_a_i)
            self.all_words.add(stay_b_i)
            for j, (stay_a_j, stay_b_j, patient_a_j) in enumerate(self.raw_data[i:]):
    ...
    def __getitem__(self, index):
        ...
        a, b, patient_a = self.raw_data[ab_index]
        c, d, patient_c = self.raw_data[cd_index]
        ...
        a = (a, demo_tensor_a, content_tensor_a, patient_a)
        b = (b, demo_tensor_b, content_tensor_b)
        c = (c, demo_tensor_c, content_tensor_c, patient_c)

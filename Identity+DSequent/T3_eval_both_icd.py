...
def evaluate_classifier(filename, nb_analogies, rd_seed):
    ...
    path_models = f"mimic_icd_T3/classif_cnn/classification_1_both_40e.pth"
    ...
    torch.save({"valid": mean(accuracy_true), "invalid": mean(accuracy_false)}, f"mimic_icd_T3/eval_cnn/evaluation_correct_{rd_seed}_both_40e.pth")
    print(f"Scores: tp {tp} tp, tn {tn}, fn {fn}, fp {fp}, f1{f1}")

if __name__ == '__main__':
    evaluate_classifier(filename = "T3_ID+DSEQ_ICD.txt", nb_analogies = 50000, rd_seed = 1)

...
if __name__ == '__main__':
    ...
    df = pd.read_csv('processed_id/notes.csv')
    df['text'] = df['text'].astype(str).apply(text2words)

    if args.phase != 'infer':
        epochs = args.epochs
        splits = range(10)
        data = json.load(open('processed_id/files/splits.json'))
        ...
        model.save('processed_id/models/doc2vec.model')
    else:
        print('Infering...')
        doc2vec = Doc2Vec.load('processed_id/models/doc2vec.model')
        ...
        json.dump(vector_dict, open('processed_id/files/vector_dict.json', 'w'))

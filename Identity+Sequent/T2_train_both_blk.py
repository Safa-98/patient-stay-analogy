...
from T2_data_blk import Task1Dataset, enrich, generate_negative
...
args.n_ehr = len(json.load(open(os.path.join('processed_blk_T2/files/demo_index_dict.json'), 'r'))) + 10
...
def train_classifier(filename, nb_analogies, epochs, rd_seed):
    ...
    torch.save({"state_dict_classification": classification_model.cpu().state_dict(), "state_dict_embeddings": embedding_model.cpu().state_dict(), "losses": losses_list, "times": times_list}, f"mimic_blk_T2/classif_cnn/classification_{rd_seed}_both_{epochs}e.pth")

if __name__ == '__main__':
    train_classifier(filename = "T2_ID+SEQ_BLK.txt", nb_analogies = 50000, epochs = 40, rd_seed = 1) 

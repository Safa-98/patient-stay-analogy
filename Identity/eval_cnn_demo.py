...
def evaluate_classifier(filename, nb_analogies, rd_seed):
    ...
    path_models = f"mimic_id/classif_cnn/classification_1_demo_40e.pth" 
    ...
    for a, b, c, d in test_dataloader:        
        ...
        print(analogy_element_a, patient_a, analogy_element_b, analogy_element_c, patient_c, analogy_element_d)
    ...
    torch.save({"valid": mean(accuracy_true), "invalid": mean(accuracy_false)}, f"mimic_id/eval_cnn/evaluation_tp_correct_{rd_seed}_demo_40e.pth")
    print(f"Scores: tp {tp} tp, tn {tn}, fn {fn}, fp {fp}")

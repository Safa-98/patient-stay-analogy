...
from utils import elapsed_timer, tpr_tnr_balacc_harmacc_f1
from data import Task1Dataset, enrich, generate_negative, generate_pos_only
from analogy_classif_demo import Classification
import cnn_dem
...
def train_classifier(filename, nb_analogies, epochs, rd_seed):
    ...
    embedding_model = cnn_dem.CNN(args)
    ...
    for epoch in range(epochs):
        ...
        with elapsed_timer() as elapsed:
            for a, b, c, d in train_dataloader:
                ...
                a = embedding_model(dd_a.to(device)) 
                b = embedding_model(dd_b.to(device))
                c = embedding_model(dd_c.to(device))
                d = embedding_model(dd_d.to(device))
...
#
    ...
    torch.save({"state_dict_classification": classification_model.cpu().state_dict(), "state_dict_embeddings": embedding_model.cpu().state_dict(), "losses": losses_list, "times": times_list}, f"mimic_id/classif_cnn/classification_{rd_seed}_demo_{epochs}e.pth")

if __name__ == '__main__':
    train_classifier(filename = "T1_IDENTITY_200.txt", nb_analogies = 50000, epochs = 40, rd_seed = 1)

...
class Task1Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filename = "T2_ID+SEQ_CAT.txt"): 
        super(Task1Dataset).__init__()
        #self.mode = mode
        self.raw_data = load_data(filename = filename)
        #self.prepare_data()
        self.set_analogy_classes()
        self.demo_dict = json.load(open(os.path.join('processed_cat_T2/files/demo_dict.json'), 'r')) #DSEQ
        self.vector_dict = json.load(open('processed_cat_T2/files/vector_dict.json', 'r'))
